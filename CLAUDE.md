...
│                            # of BLE sync (mock_orion_ble.py) and BLE firmware
│                            # update (mock_orion_ble_ota.py) against real
│                            # hardware, plus manufacturing provisioning
...
| Hand-test BLE firmware update against real Ori hardware | [tools/mock_orion_ble_ota.py](tools/mock_orion_ble_ota.py) |
...
- [x] **M5 — Firmware BLE + firmware update** — locked 2026-06-29
  GATT server (17 chars, v1.0, including Clock Face) + ANCS client (48-app icon registry, stored as LVGL indexed I1/I2/I4/I8 images via `firmware/img/ANCS_icons/convert_ancs_indexed.py` to keep flash usage down), passkey bonding, dual-connection (Orion + iPhone bond slots), connection-status border, album-art JPEG decode (TJPGD), firmware-update receiver, boot splash screen (Ori wordmark shown immediately on boot while PSRAM/BLE init runs), separate NS/DS ANCS backlog-replay queues + `g_pmeta` sizing fix. `h2zero/NimBLE-Arduino@2.5.0`. **Post-lock:** weather badge + temperature bubble on the profile card (Device Settings `"w"`/`"d"`), 12/24-hour time format toggle (Device Settings `"h"`, `time_format.cpp`), ANCS notification filter (Device Settings `"f"`). Build state: RAM 24.6%, Flash 86.0% (of 3 MB OTA slot). **Transport changed 2026-08-16:** the firmware update moved from USB CDC to BLE (chars `0014`/`0015`, `ble-protocol.md` §14) and the USB CDC receiver was deleted — the enclosure exposes no reachable data port, so a deployed unit has to update over the air. GATT is now 21 chars; `ota.md` is the authority.
