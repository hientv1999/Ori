...
| Orion pairing UX | **OS-native pairing confirmation on both platforms** — Ori displays its 6-digit passkey; the user confirms it in the Windows/macOS system Bluetooth prompt, not a custom in-app modal. Decided 2026-07-08: CoreBluetooth exposes no app-level pairing UI hook on macOS (a hard OS constraint, independent of tech stack), so a fully custom passkey modal can't be pixel-identical across platforms — standardizing on the OS prompt keeps behavior consistent instead of forking the UX per OS. `setup-flow.md`'s passkey modal is a short "confirm the code shown on Ori in the system prompt" guidance screen, not digit-entry boxes. |
