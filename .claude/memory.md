...
| Orion pairing UX | **Custom in-app passkey entry on Windows** — Ori displays its 6-digit passkey; the user types it into Orion's own digit-box modal (`setup-flow.md`'s passkey modal), which drives WinRT's `DeviceInformationCustomPairing` (`PairingRequested` → `ProvidePin`) so the OS bonds against the typed code. Revised 2026-07-08 (same-day correction of an earlier "OS-native confirm on both platforms" decision, which wrongly generalized macOS's constraint to Windows too). **macOS is unsolved, not "OS-native by design"** — CoreBluetooth exposes no app-level pairing UI hook at all (a hard OS wall, independent of tech stack), so custom entry is impossible there; since the macOS build hasn't started (M6b), its pairing UX is deferred rather than decided. When that work begins, options are: accept the system Bluetooth sheet on macOS only (inconsistent but simple), or move passkey verification to the application layer (bond "Just Works" + verify the code over a GATT characteristic instead of at the BLE link layer) for parity — the latter requires reopening the locked M1/M5 BLE contract and trades away link-layer MITM protection for an app-layer equivalent, so it needs explicit sign-off before pursuing. |
...
| Orion UI model | **Compact, movable, non-resizable panel — no separate "main window," but NOT a click-away-dismiss flyout.** Revised 2026-07-08 (supersedes the 2026-06-29 "dismissed on click-away, never taskbar-visible" spec, based on hands-on review of the real app): the panel **auto-opens on launch**, **stays visible until the user explicitly minimizes it** (via the persistent in-app minimize button — top-right of a thin custom titlebar, visible on every screen including first-run setup — or by clicking the tray icon again), and **shows a normal taskbar entry while open** (`skipTaskbar:false`) so it's alt-tab-able like a regular small app window. It's still frameless (`decorations:false`, custom-drawn rounded corners) and still tray-anchored (tray icon toggles show/hide, restores whatever position the user last dragged it to), but no longer auto-hides on focus loss, and no longer `alwaysOnTop`. The custom titlebar (`.app-titlebar` in `index.html`) doubles as the window drag handle — every screen (pairing wizard, settings, profile editor, calendar source picker, connection status) is a view inside this one panel, not a separate window. |
| Orion rounded corners (Windows) | **CSS `border-radius` + a transparent Tauri window is NOT sufficient on Windows 10** — measured on-device as zero transparency at the geometric corner (WebView2's alpha doesn't reliably reach the native window surface, and Windows 10 has no DWM auto-rounding — that's an 11-only feature). Fixed via `SetWindowRgn`/`CreateRoundRectRgn` (Win32, called from `src-tauri/src/lib.rs`'s `apply_rounded_corners()`, `windows` crate) — clips the actual HWND shape at the OS level, independent of WebView2's alpha compositing. Windows-only code, `#[cfg(target_os = "windows")]`; macOS's WKWebView is expected to handle CSS transparency correctly (not yet verified — macOS build not started). |
| Orion window drag (Windows) | The plain `data-tauri-drag-region` HTML attribute was unreliable in testing (window didn't move on a real click-drag). Fixed with an explicit handler in `app.js`: `mousedown` on `.app-titlebar` (skipping clicks on `.h-ico` buttons) calls `window.__TAURI__.window.getCurrentWindow().startDragging()` — needs the `core:window:allow-start-dragging` permission in `capabilities/default.json`. |
...
| Orion weather data source | **Open-Meteo (open-meteo.com, no API key) for current condition + temperature; Windows Location API (`Geolocator`, WinRT `Devices_Geolocation` feature) for the lat/long it needs.** Decided 2026-07-09; deferred to Phase D along with the rest of the weather-push plumbing (`ble-protocol.md` §6.4's `"w"`/`"d"`/`"u"` Device Settings fields already have a working write path — see `pc-app.md` — just no data source yet). IP-based geolocation was considered and rejected: city-level accuracy varies too widely (~1–30 km) and breaks badly on VPN/mobile connections, well outside the ~5 km target for this feature. |
| Phone icon disconnected-state design | **Neutral colour in both states (never red); a diagonal slash cut across the glyph is the sole "disconnected" signal.** Revised 2026-07 — supersedes the earlier "danger red when disconnected, no slash" design (`screen-layout.md`/`gestures.md`/`connectivity.md`). Rationale: color-only status is a legibility gap for colorblind users, and a shape change (the slash) reads correctly regardless of red/grey perception — same convention as "muted mic"/"no wifi" icons elsewhere. Implemented identically across all four surfaces: `firmware/src/widgets/widget_status_bar.cpp` (LVGL `lv_line` diagonal, real device), `Ori_UI_Prototype.html`'s `#i-phone-broken` (SVG mask-cut, pre-existing technique reused), `PC_app/Orion_UI_Prototype.html/css` and `PC_app/orion/src/index.html`/`styles.css` (both: plain overlaid SVG `<line class="phone-slash">`, shown via a `.phone-disconnected` class toggle in JS). |
