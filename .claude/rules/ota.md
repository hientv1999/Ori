...
- **BLE "quiet mode" for the whole download phase** (`ble_manager::set_ota_transfer_quiet(true)` at accepted BEGIN — the reversible little sibling of the commit-phase quiesce below). Kills the *bursts* without dropping the links:
  - **Advertising stops** and `restart_advertising()` no-ops for the duration — a bonded peer reconnecting mid-download would fire the heaviest BLE burst in the system (encryption restore → NimBLE's bonding-restored CCCD replay → auto `onSubscribe` → ANCS resync relay → Orion delta sync) right on top of the USB CDC RX path, plus bond/CCCD NVS writes whose cache-disable stalls both cores. Peers *already connected* keep their (inert) links — an idle link is nearly free.
  ...
  - **On failure-resume** (`show_error_screen` → `set_ota_transfer_quiet(false)`): advertising re-arms per bond state, ANCS resumes, and — only if events were actually dropped while suspended (iOS never re-sends missed NS events on the same connection) — the iPhone link is force-dropped so the bonded auto-reconnect replays the full backlog (the same PreExisting-replay machinery every fresh bond uses, `setup-flow.md`). Nothing dropped → both links resume warm, no ceremony. Pre-accept rejects (`too_large`/`no_memory`) never entered quiet mode; the lift is a no-op there. A successful commit never lifts it — the reboot supersedes.
