...
- Weather Alert fires once per calendar day, `weather_alert_offset_min` before `work_hours_end_min`, on a configured work day, while the current condition is Rain/Thunderstorm/Snow. Low Battery Alert fires **exactly once** per dip below `low_battery_threshold_pct`, latching until either the level recovers back above the threshold or the **phone link drops** (`set_phone_connected(false)`, which also re-arms it for the reconnect that follows). Nothing else re-arms it — in particular not an incoming notification, even though `ancs_client::queue_add()` routes every one of those through `set_phone_connected(true)` as a link-is-up signal for the status-bar icon. A `battery` of 0 is `phone_stats()`' disconnected/not-yet-read sentinel, never a real level, so it never fires the alert (otherwise every reconnect popped a bogus "0%" one before the first Battery Service read landed).
