...
- **12-/24-hour time format:** independent of the face choice, both faces honour the user's time-format preference (`g_fmt` in `time_format.cpp`, `nvs::get/set_time_format()`, default 24-hour). 24-hour shows "14:30". In 12-hour, since the XL digit-only clock font has no letters, the "AM"/"PM" can't ride inside the hour/minute text itself — Digital renders it as a small subtext to the right, via a 3-slot row `[left_spacer][center_wrap: hour+colon+minute][right_slot: AM/PM]` where `left_spacer`/`right_slot` both have `flex_grow=1` and are *always present* regardless of whether AM/PM has content — two equal-growing tracks flanking a fixed-content middle keeps `center_wrap` centered in the panel width no matter what's in `right_slot`. (An earlier `LV_OBJ_FLAG_IGNORE_LAYOUT` + manual per-tick realign attempt, and then a simpler "just make it a flex sibling of minute and accept the centering shift" attempt, both preceded this — the first didn't render on hardware despite checking out against the LVGL source, the second worked but visibly shifted hour:minute's position; this is the version that satisfies both mechanically-verified and visually-stable.) The date strip below has no AM/PM suffix in either mode, so it stays centered regardless. Analog never shows AM/PM either (no digital time readout on the dial to pair a suffix with) — both faces' date strips read `"WEDNESDAY, MAY 14, 2026"` (day, month, day-of-month, year). Settable over BLE via **Device Settings** (`ble-protocol.md` §3/§6.4, char `000E`, key `"h"`, 0=24-hour 1=12-hour), applied + persisted immediately, and via the `ORI_DEBUG_SERIAL` cycler (`H` toggles). The same preference also drives the status-bar clock, meeting-list times, and ANCS notification timestamps.
