...
- **Passkey confirmation UI**: OS-native, not a custom in-app modal (decided 2026-07-08 — `memory.md`). LE Secure Connections Passkey Entry means Ori displays a 6-digit code; the user confirms it in the Windows/macOS system Bluetooth pairing prompt. CoreBluetooth exposes no app-level pairing UI hook on macOS, so a custom digit-entry modal can't be pixel-identical across platforms — Orion's own modal is a short "confirm the code shown on Ori in the system prompt" guidance screen that's showing while the OS prompt is up, not a data-entry form.
