...
- **Controls-mode OS bridge**: subscribes to the `Keyboard Command` notify characteristic and translates each command to the appropriate OS action:
  ...
  - `seek arg:N` → seek the active session to N seconds (Windows: `GlobalSystemMediaTransportControlsSession::TryChangePlaybackPositionAsync`; macOS: `MRMediaRemoteSetElapsedTime`)
  - `vol_set arg:N` → set OS master volume (Windows: `IAudioEndpointVolume::SetMasterVolumeLevelScalar(N/100.0)`; macOS: `AudioObjectSetPropertyData kAudioHardwareServiceDeviceProperty_VirtualMainVolume`), then **write `HostVolumeState{level:N}` back to Ori** as confirmation of the value actually applied. Ori ignores this write-back during its own 800 ms swipe-override window (`ble-protocol.md` §12) — Orion does not need to suppress anything on its side, just always write back.
