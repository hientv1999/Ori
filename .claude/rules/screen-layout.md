...
- Contents left → right:
  ...
  - **Mode-toggle button** — rightmost element always. Cycles left panel between **Calendar** and **Controls**. **Hidden until Orion has fully synced** (not merely connected — a bonded reconnect keeps it hidden through the Reconnect-Syncing window) — except in Clock or Calendar view, where it acts as a return button (calendar icon, neutral style). Icon always shows the destination. See `media-mode.md`.

## Right Panel — Profile Card

Always visible. Contents:
- Circular profile photo, **228 × 228 px** (or Ori wordmark if no photo) — **border colour reflects Orion connection status** (see below)
...
### Profile-photo border — Orion connection status indicator

6 px border + a static (non-animated) soft glow in the same colour encodes Ori's own BLE connection state to Orion — firmware-derived from Device Status (`ble-protocol.md` §3), not something Orion pushes over the wire. **Disconnected shows the border with no glow** — a glow on the grey/no-signal state would read as a render glitch rather than a deliberate status:

| Device Status | State | Border colour |
|---|---|---|
| `RUNTIME_READY` | Connected | `#92C353` (green) |
| `RUNTIME_RECONNECTING` / `RUNTIME_SYNCING` (and the equivalent setup-time syncing states) | Connecting / syncing | `#FFAA44` (yellow) |
| No BLE connection to Orion | Disconnected | `#8A8884` (grey) |

**Device-side fallback:** there is no fallback needed beyond this — the border always reflects Ori's own live connection state directly, so there's no "can't verify" case the way Teams presence used to have.

Border colour transitions animate at ~300 ms ease.

### Weather icon + temperature text

Two small elements sit just outside the profile-photo circle — no bubble/background on either, and both are positioned clear of the photo so they never overlap it — pushed by Orion via the Device Settings characteristic (`"w"`/`"d"`/`"u"` fields — `ble-protocol.md` §3/§4/§6.4):

- **Weather icon** — top-left corner, bare condition glyph with no background or border. Seven conditions: Clear, Partly Cloudy, Cloudy, Rain, Thunderstorm, Snow, Fog. Every glyph is built from plain circles, rounded rects, and line strokes (no bezier art) so it maps directly to LVGL primitives (`lv_obj` circles/rounded-rects, `lv_line`) — no bitmap asset needed. Reference implementation: `WEATHER_ICONS` in `Ori_UI_Prototype.js`. Each condition also has a day/night variant (Device Settings `"n"`, `ble-protocol.md` §4/§6.4): Clear/Partly Cloudy swap the sun disc for a moon disc plus a few background stars; the remaining cloud/fog-bearing conditions have no sun/moon element to swap, so they instead darken their cloud/fog-line color at night. Rain, Thunderstorm, Snow, and Fog additionally vary by intensity (Device Settings `"i"` — Light/Moderate/Heavy for the first three, Light/Heavy only for Fog, no intensity axis for Clear/Partly Cloudy/Cloudy) via stroke count: 1/2/3 raindrops, snowflakes, or lightning bolts.
...
**Device-side fallback:** "don't show what can't be verified" — before the first Device Settings write containing `"w"`/`"d"`/`"u"`, and whenever the BLE-PC link is down, both elements are hidden entirely (no placeholder glyph, no stale reading). They reappear once Orion reconnects and re-pushes.
