...
// exception on some runs (known M4 bug, not yet fixed). Always call via
// a deferred lv_timer or from the main loop after the callback returns.
// The LVGL modal_factory_reset.cpp Reset button already defers via a
// one-shot lv_timer; the BLE path uses the ble_manager event queue.
void execute();
