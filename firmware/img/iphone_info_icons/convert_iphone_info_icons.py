"""
iPhone Info/Stats modal icon build pipeline — crop to 28x28 then convert to
LVGL C arrays. Same technique as firmware/img/shortcut_icons/
convert_shortcut_icons.py (crop+invert then LVGLImage.py), separate output
...
Usage:
    python convert_iphone_info_icons.py
...
# ── Tunable parameters ────────────────────────────────────────────────────────
SIZE        = (70, 70)          # native 1:1 with the modal's 70px icon box
                                # (.ip-icon-wrap / modal_iphone_info ICON_BOX) —
                                # was 28 + a 2.5x LVGL upscale, which rendered
                                # visibly soft; bake at display size instead
                                # and draw with no transform.
...
def convert(src, token, cname, lvgl_script):
    ...
    try:
        ...
        dest = os.path.join(OUTPUT_DIR, f"iphoneinfo_{cname}.c")
        shutil.move(os.path.join(tmp, c_files[0]), dest)
        print(f"  converted {os.path.basename(src)} -> iphoneinfo_{cname}.c  ({COLOR_FMT}, compress={COMPRESS})")
    finally:
...
def main():
    ...
    if not images:
        print("No PNG/JPG files found in iphone_info_icons/")
        sys.exit(0)
