...
def crop(src: str) -> str:
    ...
    img = Image.open(src).convert("RGBA" if ext == ".png" else "RGB")
    img = img.resize(SIZE, Image.LANCZOS)
    img.save(src)
    print(f"  cropped   {os.path.basename(src)} -> {SIZE[0]}×{SIZE[1]}")
    return src
...
def convert(src: str, token: str, lvgl_script: str):
    ...
    try:
        # LVGLImage.py only accepts PNG. Re-save any non-PNG as PNG first.
        ext = os.path.splitext(src)[1].lower()
        if ext in (".jpg", ".jpeg"):
            png_path = os.path.join(tmp, token + ".png")
...
def main():
    ...
    for path in images:
        ...
        crop(path)
        convert(path, token, lvgl_script)
